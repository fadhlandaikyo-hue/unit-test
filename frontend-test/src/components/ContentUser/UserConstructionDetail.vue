...
import { useLocalStorage } from "../ContentHome/composables/useLocalStorage.js"
import "../ContentHome/constructiondetail.css"
import UserButtonBackConstruction from "../Utilities/UtilitiesUsers/UserButtonBackConstruction.vue";
import UserProgressBarDetail from "../UserComponents/UserProgressBarDetail.vue";

const route = useRoute()
const { saveOne, getOne, saveProjectDate, getProjectDate } = useLocalStorage()
...
async function confirmSave() {
  if (passwordInput.value.trim() === CORRECT_PASSWORD) {
    ...
    if (pendingSaveType.value === "date" || pendingSaveType.value === "both") {
      try {
        await saveProjectDate(project.value.id, draftStartDate.value, draftEndDate.value)
        project.value.completionDate = `${draftStartDate.value} - ${draftEndDate.value}`
      } catch (e) {
        passwordError.value = "Tanggal gagal disimpan. Pastikan migration backend sudah dijalankan."
        return
      }
    }
      ...
      <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="space-y-4">
          ...
          >
            ...
            <UserProgressBarDetail v-model="project.draft" class="mb-4" />
